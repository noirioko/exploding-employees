...
import { yuwonMessages } from '../data/yuwonMessages';
import { minkyuMessages } from '../data/minkyuMessages';

function FloatingEmployee({ character = 'yuwon' }) {
  // Select messages based on character
  const messages = character === 'minkyu' ? minkyuMessages : yuwonMessages;
  ...
  const [chosenConsequence, setChosenConsequence] = useState(null);
  const [consequenceInProgress, setConsequenceInProgress] = useState(false);
  ...
  const preloadedExplosionSoundRef = useRef(null);
  ...
  useEffect(() => {
    ...
    const audio = new Audio('/sounds/roblox-explosion-sound.mp3');
    audio.preload = 'auto';
    audio.volume = 0.5;
    preloadedExplosionSoundRef.current = audio;
  }, [character]);
  ...
  // Messages now come from external data file
  ...
  const handleClick = () => {
    if (isSleeping || isExploding || consequenceInProgress) return;
    ...
    let messageObj;
    if (newClickCount <= messages.presetSequence.length) {
      // Clicks 1-10: Preset sequence
      messageObj = messages.presetSequence[newClickCount - 1];
      showDialogueMessage(messageObj.text, messageObj.expression, 3000);
      playSound(messageObj.expression);
    } else if (newClickCount === messages.presetSequence.length + 1) {
      // Click 11: Choose which consequence will happen
      const roll = Math.random();
      let consequence;
      if (roll < 0.25) {
        consequence = 'spinAway';
      } else if (roll < 0.5) {
        consequence = 'tvShutoff';
      } else if (roll < 0.75) {
        consequence = 'sleep';
      } else {
        consequence = 'explode';
      }
      setChosenConsequence(consequence);

      // Show random secondary warning
      const randomIndex = Math.floor(Math.random() * messages.secondaryWarnings.length);
      messageObj = messages.secondaryWarnings[randomIndex];
      showDialogueMessage(messageObj.text, messageObj.expression, 3000);
      playSound(messageObj.expression);
    } else if (newClickCount <= messages.presetSequence.length + 10) {
      // Clicks 12-20: Random secondary warnings
      const randomIndex = Math.floor(Math.random() * messages.secondaryWarnings.length);
      messageObj = messages.secondaryWarnings[randomIndex];
      showDialogueMessage(messageObj.text, messageObj.expression, 3000);
      playSound(messageObj.expression);
    } else if (newClickCount <= messages.presetSequence.length + 13) {
      // Clicks 21-23: Random foreshadowing messages
      const foreshadowArray = messages.foreshadowing[chosenConsequence];
      const randomIndex = Math.floor(Math.random() * foreshadowArray.length);
      messageObj = foreshadowArray[randomIndex];
      showDialogueMessage(messageObj.text, messageObj.expression, 3000);
      playSound(messageObj.expression);
    } else {
      // Click 24: Trigger the chosen consequence!
      setConsequenceInProgress(true);
      if (chosenConsequence === 'spinAway') {
        setTimeout(() => triggerSpinAway(), 1000);
      } else if (chosenConsequence === 'tvShutoff') {
        setTimeout(() => triggerTVShutoff(), 1000);
      } else if (chosenConsequence === 'sleep') {
        setTimeout(() => triggerSleep(), 1000);
      } else {
  ...
  const triggerSpinAway = () => {
    setShowDialogue(false);
    if (yuwonRef.current) {
      yuwonRef.current.style.animation = 'spin-out 1s ease-in forwards';
    }
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        setTimeout(() => {
          ...
          setClickCount(0);
          setConsequenceInProgress(false);
          showDialogueMessage("...Fine. I'm back.", 'neutral', 3000);
        }, 800);
      }, 10000);
    }, 1000);
  };

  const triggerTVShutoff = () => {
    setShowDialogue(false);
    if (yuwonRef.current) {
      yuwonRef.current.style.animation = 'tv-shutoff 0.8s ease-out forwards';
    }

    setTimeout(() => {
      setYuwonHidden(true);
      // Come back after 15 seconds
      setTimeout(() => {
        setYuwonHidden(false);
        if (yuwonRef.current) {
          yuwonRef.current.style.animation = 'tv-turnon 0.5s ease-in forwards';
        }
        setTimeout(() => {
          setClickCount(0);
          setConsequenceInProgress(false);
          showDialogueMessage("*blinks* What... what just happened?", 'oh', 3000);
        }, 500);
      }, 15000);
    }, 800);
  };
  ...
  const triggerSleep = () => {
    ...
    // 1 hour sleep
    sleepTimerRef.current = setTimeout(() => wakeUp(), 3600000);
  };
  ...
  const triggerExplosion = async () => {
    ...
    // Change to superangry sprite DIRECTLY (no re-render lag!)
    if (yuwonRef.current) {
      const superAngryPath = character === 'minkyu'
        ? '/images/working_employees/cornerminkyu_sad.png'  // Minkyu doesn't have superangry, use sad
        : '/images/working_employees/corneryuwon_superangry.png';
      yuwonRef.current.src = superAngryPath;
    ...
    // ALL EXPLOSION EFFECTS IN ONE SYNCHRONOUS BLOCK - NO DELAYS!
    // Play sound
    if (preloadedExplosionSoundRef.current) {
      preloadedExplosionSoundRef.current.currentTime = 0;
      preloadedExplosionSoundRef.current.play().catch(err => console.log('Sound failed:', err));
    }
    // Hide Yuwon
    setYuwonHidden(true);
    // Show GIF
    setShowExplosionGif(true);
    // Explode sprite into pieces
    if (imageRef) {
    ...
    if (yuwonRef.current) {
      yuwonRef.current.classList.remove('enlarge-and-tremble', 'crying-shrink');
    }
  };

  const startDeathTimer = () => {
    setDeathTimer(3600); // 1 hour in seconds
  ...
  const bringBackYuwon = () => {
    ...
    if (yuwonRef.current) {
      ...
      yuwonRef.current.style.opacity = '';
      yuwonRef.current.style.transform = '';
    }
    ...
    <div style={{ position: 'fixed', bottom: '90px', right: '20px', zIndex: 9999 }}>
      <style>
        {`
          ...
          .crying-shrink {
            animation: crying-shrink 0.5s ease-out forwards !important;
          }
