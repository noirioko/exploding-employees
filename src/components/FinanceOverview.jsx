...
function FinanceOverview() {
  const { completedTasks, deleteCompletedTask, updateCompletedTask, budgetGoals, setBudgetGoals } = useApp();
  ...
  const getSpendingByCategory = () => {
    const monthTasks = getFinanceByMonth().filter(t => t.amount < 0);
    const categoryTotals = {};

    monthTasks.forEach(task => {
      const category = task.financeCategory || 'other-outcome';
      if (!categoryTotals[category]) {
        categoryTotals[category] = 0;
      }
      categoryTotals[category] += Math.abs(task.amount);
    });
  ...
  const getCategoryLabel = (category) => {
    ...
    return categories[category] || category || 'No category';
  };
  ...
  const handleSaveEdit = () => {
    updateCompletedTask(editingId, editLog);
    setEditingId(null);
    setEditLog(null);
  };
  ...
  return (
    <div className="content">
      <h2 style={{ fontSize: '24px', color: '#00897b', marginBottom: '20px', fontWeight: 700 }}>
        💰 Finance Overview
      </h2>
      ...
      {spendingByCategory.length > 0 && (
        <div style={{
          ...
          <h3 style={{ fontSize: '18px', color: '#00897b', marginBottom: '15px' }}>
            📊 Spending Breakdown ({financeMonth.toLocaleDateString('en-US', { month: 'long' })})
          </h3>
            ...
            {spendingByCategory.map(({ category, amount }) => {
              ...
              return (
                <div key={category} style={{ display: 'flex', alignItems: 'center', gap: '10px' }}>
                  <div style={{ minWidth: '180px', fontSize: '13px', color: '#666' }}>
                    {getCategoryLabel(category)}
                  </div>
                  <div style={{ flex: 1, background: '#f5f5f5', borderRadius: '4px', overflow: 'hidden', height: '24px', position: 'relative' }}>
                    <div style={{
                      width: `${percentage}%`,
                      height: '100%',
          ...
          {spendingByCategory.length > 0 && (
            <div style={{ marginTop: '15px', padding: '10px', background: '#f9f9f9', borderRadius: '6px', fontSize: '12px', color: '#666' }}>
              💡 Your biggest expense this month: <strong>{getCategoryLabel(spendingByCategory[0].category)}</strong> ({spendingByCategory[0].amount.toLocaleString()} ₩)
            </div>
          )}
        </div>
      )}

      {/* Finance Records with Month Navigation */}
      <div>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '20px' }}>
          <h3 style={{ fontSize: '18px', color: '#00897b' }}>💰 Finance Records</h3>
        ...
        {/* Finance Table */}
        <div className="tasks-table finance-overview-table">
            ...
            {getFinanceByMonth().length === 0 ? (
              ...
              getFinanceByMonth().map(log => {
                ...
                if (isEditing) {
                  // Edit mode
                  return (
                    <div key={log.id} className="task-row" style={{ background: '#fff9e6' }}>
                      ...
                      <div className="col-deadline">
                        ...
                        >
                          ...
                          <optgroup label="Expenses">
                            ...
                            <option value="life">🏠 Life (Bills, Rent)</option>
                            <option value="pets">🐾 Pets</option>
                            <option value="fun-games">🎮 Fun & Games</option>
                            <option value="gacha">🎰 Gacha & Gambling</option>
                            <option value="impulse">💸 Impulse Buy</option>
                ...
                return (
                  <div key={log.id} className="task-row">
                    ...
                    <div className="col-deadline" style={{ fontSize: '12px', color: '#666' }}>
                      {getCategoryLabel(log.financeCategory)}
                    </div>
                    ...
                    <div className="col-actions">
                      ...
                      <button className="delete-btn task-btn" onClick={() => deleteCompletedTask(log.id)}>
                        ✕
                      </button>
                    </div>
                  </div>
        ...
        <div style={{ marginTop: '15px', display: 'flex', gap: '20px', justifyContent: 'flex-end' }}>
          <div style={{ fontSize: '14px', color: '#666' }}>
            <strong>Total Income:</strong> <span style={{ color: '#4caf50', fontWeight: 700 }}>
              +{getFinanceByMonth().filter(t => t.amount > 0).reduce((sum, t) => sum + parseFloat(t.amount || 0), 0).toLocaleString()} ₩
            </span>
          </div>
          <div style={{ fontSize: '14px', color: '#666' }}>
          ...
          <div style={{ fontSize: '14px', color: '#666' }}>
            <strong>Net:</strong> <span style={{ fontWeight: 700, color: getFinanceByMonth().reduce((sum, t) => sum + parseFloat(t.amount || 0), 0) >= 0 ? '#4caf50' : '#ff5252' }}>
              {getFinanceByMonth().reduce((sum, t) => sum + parseFloat(t.amount || 0), 0).toLocaleString()} ₩
            </span>
          </div>
        </div>
      </div>
